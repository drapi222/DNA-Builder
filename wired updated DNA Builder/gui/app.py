...
# ---------------------------------------------------------------------------
# Reference structure superimposition
# ---------------------------------------------------------------------------

@app.route("/api/reference_pdb", methods=["POST"])
def api_reference_pdb():
    """Build a canonical reference and superimpose it onto the current structure."""
    from dna_builder.classifier import _match_atoms_by_identity, _kabsch_rmsd

    data = request.get_json()
    sequence     = data.get("sequence", "").upper().strip()
    form         = data.get("form", "B").upper()
    pdb_content  = data.get("pdb", "")
    raw_content  = data.get("content", "")
    raw_filename = data.get("filename", "structure.cif")
    ...
    if form not in ("A", "B", "Z"):
        return jsonify({"error": f"Invalid form: {form}"}), 400

    try:
        # Build canonical reference
        try:
            ref_atoms = build_dna_v2(sequence, form)
        except Exception:
            ref_atoms = build_dna(sequence, form)

        # Superimpose onto input structure when provided
        if pdb_content or raw_content:
            if pdb_content:
                suffix_inp, inp_content = ".pdb", pdb_content
            else:
                ext = os.path.splitext(raw_filename)[1].lower() or ".cif"
                suffix_inp, inp_content = ext, raw_content

            with tempfile.NamedTemporaryFile(mode="w", suffix=suffix_inp, delete=False) as fh:
                fh.write(inp_content)
                tmp_inp = fh.name
            try:
                inp_atoms = parse_structure(tmp_inp)
            finally:
                os.unlink(tmp_inp)

            chain_ids = sorted(set(a.chain_id for a in inp_atoms))
            match_chain = None if len(chain_ids) > 1 else (chain_ids[0] if chain_ids else "A")

            inp_coords, ref_coords, matched = _match_atoms_by_identity(
                inp_atoms, ref_atoms, chain=match_chain)
            if matched < 5:
                inp_coords, ref_coords, matched = _match_atoms_by_identity(
                    inp_atoms, ref_atoms, chain=chain_ids[0] if chain_ids else "A")

            if matched >= 3:
                centroid_P = inp_coords.mean(axis=0)
                centroid_Q = ref_coords.mean(axis=0)
                _, R, _ = _kabsch_rmsd(inp_coords, ref_coords)
                # Inverse transform: move reference into the input's coordinate frame.
                # Forward Kabsch maps P→Q via R@x+t; inverse maps Q→P via R.T@(y-centroid_Q)+centroid_P
                for atom in ref_atoms:
                    c = np.array([atom.x, atom.y, atom.z])
                    c_new = R.T @ (c - centroid_Q) + centroid_P
                    atom.x, atom.y, atom.z = float(c_new[0]), float(c_new[1]), float(c_new[2])

        buf = io.StringIO()
        write_pdb(ref_atoms, buf, f"{form}-DNA reference 5'-{sequence}-3'")
        return jsonify({"pdb": buf.getvalue(), "ok": True})
    except Exception as exc:
        return jsonify({"error": str(exc)}), 400
