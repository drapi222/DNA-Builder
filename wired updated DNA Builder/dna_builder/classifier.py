...
from .io_parser import parse_structure, extract_sequence_from_atoms, detect_format, clean_atoms
...
def classify_structure(filepath: str, verbose: bool = True,
    ...
    if not is_xyz:
        # Real PDB/CIF depositions carry waters, counter-ions, and
        # crystallization additives (HOH, MG, SPM, ...) alongside the DNA/RNA
        # chains; strip them so sequence/chain extraction below only sees
        # nucleotide residues. XYZ atoms have no residue names to filter on.
        input_atoms = clean_atoms(input_atoms)
        if not input_atoms:
            raise ValueError(
                f"No nucleotide atoms found in {filepath} after removing "
                "waters/ions/ligands."
            )
