...
## Show the headset view on the laptop

Start the same VR server with `npm run dev`, then open this on the laptop:

```text
http://localhost:8081/mirror
```

Keep the mirror page open while the Quest opens the normal VR app:

```text
http://localhost:8081
```

The Quest page publishes the current DNA model, molecule pose, selected spin
axis, and headset camera pose to the local server. The laptop mirror subscribes
to those updates and renders a live spectator view for demos or screen
recording. Use **Fullscreen mirror** on the laptop for a clean display.
