...
# Recognized nucleotide residue names, used to strip waters/ions/ligands from
# real deposited PDB files. DA/DT/DG/DC are DNA; bare A/U/G/C and RA/RU/RG/RC
# are RNA (placeholders — this builder doesn't build RNA yet, but structures
# containing RNA residues can still be classified/viewed once cleaned).
NUCLEOTIDE_RESNAMES = set(RESIDUE_NAMES.values()) | {
    "A", "U", "G", "C", "RA", "RU", "RG", "RC",
}
...
def parse_pdb(filepath: str) -> List[Atom]:
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            # Alternate conformations: keep only the blank/primary ('A') altLoc
            alt_loc = line[16:17]
            if alt_loc not in (' ', 'A'):
                continue
...
def parse_cif(filepath: str) -> List[Atom]:
    ...
    for row in data_rows:
        ...
        # Alternate conformations: keep only the unset/primary ('A') altloc
        alt_id = _get(row, 'label_alt_id', '.')
        if alt_id not in ('.', 'A'):
            continue
...
def clean_atoms(atoms: List[Atom]) -> List[Atom]:
    """
    Strip non-nucleotide atoms (waters, counter-ions, crystallization
    additives such as spermine/glycerol) from a parsed structure.

    Real PDB depositions include HETATM records (HOH, MG, NA, SPM, ...)
    alongside the DNA/RNA chains; these break sequence extraction and
    chain detection if left in, so keep only recognized nucleotide
    residues.
    """
    return [a for a in atoms if a.residue_name.strip() in NUCLEOTIDE_RESNAMES]
