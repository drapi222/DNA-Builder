...
const BOND_OUTLINE_RADIUS = 0.075;
...
function rotateDNA(radians) {
  ...
  dnaRoot.rotateOnAxis(axis, radians);
}
...
function resetDNA() {
  ...
  setStatus(modelGroup ? 'Model pose reset.' : 'Scene ready. Building a sample…');
}
...
function applyAtomColors() {
  if (!currentPDBText) return;
  loadPDB(currentPDBText, { resetPose: false });
}
...
function createOutlineMaterial(side = THREE.FrontSide) {
  return new THREE.MeshBasicMaterial({
    color: OUTLINE_COLOR,
    side,
    toneMapped: false,
  ...
  buckets.forEach(({ color, matrices }) => {
    if (outline) {
      group.add(createMeshFromMatrices(
        ...
        createOutlineMaterial(outlineSide),
        matrices,
        outlineScale,
      ));
    }
...
async function main() {
  ...
  ui.viewPreset.addEventListener('change', () => applyViewPreset(ui.viewPreset.value));
